...
public class OtpService {
    ...
    @Value("${spring.security.otp.expiration-ms}")
    ...
    public ResponseEntity<ResponseData<String>> verifyOtp(String email, String otp) {
        ...
        if(value == null) return new ResponseEntity<>(new ResponseData<>(null, false, "No pending record found!!!"), HttpStatus.CONFLICT);
        if(value.getExpirationTime().isBefore(LocalDateTime.now())) return new ResponseEntity<>(new ResponseData<>(null, false, "OTP Expired"), HttpStatus.CONFLICT);
